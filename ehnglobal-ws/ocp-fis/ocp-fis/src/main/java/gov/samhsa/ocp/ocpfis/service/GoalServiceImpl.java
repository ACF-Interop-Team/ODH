...
public class GoalServiceImpl implements GoalService {
    ...
    public GoalServiceImpl(ModelMapper modelMapper, IGenericClient fhirClient, FisProperties fisProperties,
            FhirValidator fhirValidator, LookUpService lookUpService, ProvenanceUtil provenanceUtil,
    ...
    @Override
    public PageDto<GoalDto> getGoalbyPatientId(String patientId, Optional<String> searchKey,
            Optional<String> searchValue, Optional<Boolean> showInactive, Optional<Integer> pageNumber,
            Optional<Integer> pageSize, Optional<Boolean> showAll) {
        int numberPerPage = PaginationUtil.getValidPageSize(fisProperties, pageSize, ResourceType.Goal.name());
        ...
        if (pageNumber.isPresent() && pageNumber.get() > 1 && otherPageBundle.getLink(Bundle.LINK_NEXT) != null) {
            firstpage = false;
            otherPageBundle = PaginationUtil.getSearchBundleAfterFirstPage(fhirClient, fisProperties, firstPageBundle,
                    pageNumber.get(), numberPerPage);
        }
        ...
        return new PageDto<>(goalDtoList, numberPerPage, totalPages, currentPage, goalDtoList.size(),
                otherPageBundle.getTotal());
    }

    @Override
    public PageDto<GoalDto> getAllGoals(Optional<Boolean> showInactive, Optional<Integer> page,
            Optional<Integer> size) {
        int numberPerPage = PaginationUtil.getValidPageSize(fisProperties, size, ResourceType.Goal.name());
        IQuery goalIQuery = fhirClient.search().forResource(Goal.class);
        goalIQuery = FhirOperationUtil.setLastUpdatedTimeSortOrder(goalIQuery, true);
        Bundle firstPageBundle;
        Bundle otherPageBundle;
        ...
        firstPageBundle = PaginationUtil.getSearchBundleFirstPage(goalIQuery, numberPerPage, Optional.empty());
    ...
    @Override
    public void createGoal(GoalDto goalDto, Optional<String> planid, Optional<String> practitionerid, String requester,
            Optional<String> userorgid) {
        List<String> idlist = new ArrayList<>();
        Goal goalreference = convertToGoal(goalDto);

        // Validate
        FhirOperationUtil.validateFhirResource(fhirValidator, goalreference, Optional.empty(), ResourceType.Goal.name(),
                "Create Goal");

        // Create
        MethodOutcome serverResponse = FhirOperationUtil.createFhirResource(fhirClient, goalreference,
                ResourceType.Goal.name());
        ...
        // Ids
        if (planid.isPresent() && !planid.get().isEmpty() && goalDto.getSubject() != null
                && !goalDto.getSubject().getReference().isEmpty()
                && !FhirOperationUtil.getFhirId(serverResponse).isEmpty()) {
            inserTasksbyActions(planid.get(),
                    goalDto.getSubject().getReference(),
                    FhirOperationUtil.getFhirId(serverResponse),
                    practitionerid.orElse(""),
                    goalDto,
                    requester,
                    userorgid);
        } else {
    ...
    @Override
    public void updateGoal(GoalDto goalDto, Optional<String> planid, Optional<String> practitionerid,
            Optional<String> userorgid) {
        List<String> idlist = new ArrayList<>();
        Goal goalreference = convertToGoal(goalDto);
        // Validate
        ...
        // Update
        MethodOutcome serverResponse = FhirOperationUtil.updateFhirResource(fhirClient, goalreference,
                ResourceType.Goal.name());
        ...
        if (planid.isPresent() && !planid.get().isEmpty()) {
            inserTasksbyActions(planid.get(),
                    goalDto.getSubject().getReference(),
                    FhirOperationUtil.getFhirId(serverResponse),
                    practitionerid.isPresent() ? practitionerid.get() : null,
                    goalDto,
                    "",
                    userorgid);
        } else {
    ...
    private GoalDto maponegoal(Goal goal) {
        ...
        List<Coding> codingscategories = goal.getCategory().get(0).getCoding().stream().map(categories -> {
            ...
            coding.setDisplay(categories.getCode().substring(0, 1).toUpperCase()
                    + categories.getCode().substring(1).toLowerCase());
            return coding;
        }).collect(toList());
    ...
    public Goal convertToGoal(GoalDto goalDto) {
        ...
        // LifeCycleStatus
        goal.setLifecycleStatus(
                Goal.GoalLifecycleStatus.valueOf(goalDto.getLifecycleStatus().toUpperCase(Locale.ROOT)));

        // Achievement Status
        List<org.hl7.fhir.r4.model.Coding> codingsachievements = goalDto.getAchievementStatus().getCoding().stream()
                .map(coding -> {
        ...
        // Description
        List<org.hl7.fhir.r4.model.Coding> codingsdescription = goalDto.getDescription().getCoding().stream()
                .map(codedescription -> {
    ...
    public void inserTasksbyActions(String planid, String patientid, String goalid, String practitioner,
            GoalDto goalDto, String requester, Optional<String> userorgid) {
        ...
        Bundle careplanBundle = fhirClient.search().forResource(CarePlan.class)
        ...
        if (careplanBundle.getEntry().size() > 0 && !careplanBundle.getEntry().isEmpty()) {
            ...
            insertDatatoCarePlan(planDefinition, existingcareplan, patientid, goalid, requester, practitioner,
                    userorgid.orElse(""));
        } else {
            ...
            insertDatatoCarePlan(planDefinition, carePlan, patientid, goalid, requester, practitioner,
                    userorgid.orElse(""));
        }
    }

    public void insertDatatoCarePlan(PlanDefinition planDefinition,
...
//                .execute();
        ...
        Bundle search_activity_goal = fhirClient.search().forResource(ActivityDefinition.class)
                .where(new ReferenceClientParam("publisher").hasId(userorgid))
                .where(new TokenClientParam("name").exactly().code("goal-task"))
                .returnBundle(Bundle.class)
                .execute();
        ...
        if (!userorgid.isEmpty()) {
            ...
            if (search_activity_goal.isEmpty() || search_activity_goal.getEntry().size() == 0) {
                ...
                try {
                    ...
                    activityreference
                            .setReference(ResourceType.ActivityDefinition.name() + "/" + outcome.getId().getIdPart());
                ...
                for (int x = 0; x < search_activity_goal.getEntry().size(); x++) {
                    ActivityDefinition resource = (ActivityDefinition) search_activity_goal.getEntry().get(x).getResource();;
                    log.info("Activity #" + x);
                    log.info("Activity Name: " + resource.getName());
                    log.info("Activity Code: " + resource.getCode());
                    log.info("Activity Title: " + resource.getTitle());
                    log.info("Activity id: " + resource.getIdElement().getIdPart());
                }
                ActivityDefinition activityDefinition = (ActivityDefinition) search_activity_goal.getEntry().get(0)
                        .getResource();
        ...
        if (planDefinition != null) {
            for (PlanDefinition.PlanDefinitionActionComponent action : planDefinition.getAction()) {
                ...
                if (!practitioner.isEmpty()) {
                    ...
                    if (practitionerDto.getPractitionerRoles() != null
                            && !practitionerDto.getPractitionerRoles().isEmpty()) {
                        if (practitionerDto.getPractitionerRoles().get(0).getPractitioner() != null) {
                            if (practitionerDto.getPractitionerRoles().get(0).getPractitioner().getDisplay() != null) {
                                owner_reference.setDisplay(
                                        practitionerDto.getPractitionerRoles().get(0).getPractitioner().getDisplay());
                            }
                        }
                    }
                ...
                MethodOutcome serverResponse = FhirOperationUtil.createFhirResource(fhirClient, task,
                ...
                carePlanActivityComponent.setOutcomeReference(taskreferenceslist)
                        .setDetail(carePlanActivityDetailComponent);
                carePlanActivityComponents.add(carePlanActivityComponent);
            }
            existingcareplan.setActivity(carePlanActivityComponents);
            MethodOutcome careplanupdateoutcome = FhirOperationUtil.updateFhirResource(fhirClient, existingcareplan,
                    "Update CarePlan");
    ...
    public String createCarePlan(GoalDto goalDto, String practitioner) {
        ...
        FhirOperationUtil.validateFhirResource(fhirValidator, carePlan, Optional.empty(), ResourceType.CarePlan.name(),
                "Create CarePlan");
        MethodOutcome methodOutcome = FhirOperationUtil.createFhirResource(fhirClient, carePlan,
    ...
    public void updateCarePlanwithGoals(String patientid, String goalid) {
        ...
        CarePlan existingcareplan = !careplanBundle.isEmpty() && !careplanBundle.getEntry().isEmpty()
                && careplanBundle.getEntry().stream().findFirst().isPresent()
                && !careplanBundle.getEntry().stream().findFirst().get().isEmpty()
                && !careplanBundle.getEntry().stream().findFirst().get().getResource().isEmpty()
                        ? (CarePlan) careplanBundle.getEntry().stream().findFirst().get().getResource()
                        : null;
