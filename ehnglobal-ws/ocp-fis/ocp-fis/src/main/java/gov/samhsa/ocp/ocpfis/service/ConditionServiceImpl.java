...
public class ConditionServiceImpl implements ConditionService {
    ...
    public ConditionServiceImpl(IGenericClient client, FisProperties fisProperties, FhirValidator fhirValidator) {
        this.client = client;
    ...
    public PageDto<ConditionDto> getConditionsforPatient(String patient, Optional<Integer> pagesize, Optional<Integer> pagenumber) {
        int numberperpage = PaginationUtil.getValidPageSize(fisProperties,pagesize, ResourceType.Condition.name());

        IQuery query = client.search().forResource(Condition.class)
                .where(new ReferenceClientParam("subject").hasId("Patient/"+patient));
        ...
        if(pagenumber.isPresent() && pagenumber.get() > 1 && otherpage.getLink(Bundle.LINK_NEXT) != null) {
            first = false;
            otherpage = PaginationUtil.getSearchBundleAfterFirstPage(client, fisProperties, firstpage, pagenumber.get(), numberperpage);
        }
    ...
    public ConditionDto saveconditionforpatient(ConditionDto conditionDto) {
        ...
        Optional<ValueSet.ValueSetExpansionContainsComponent> valueset_contains = valueSet.getExpansion()
                .getContains().stream().filter((v) -> v.getCode().equalsIgnoreCase(conditionDto.getConditioncode())).findAny();
        ...
        Optional<ValueSet.ValueSetExpansionContainsComponent> valueset_contains_priorities = priorities.getExpansion()
                .getContains().stream().filter((v) -> v.getCode().equalsIgnoreCase(conditionDto.getPriority())).findAny();
    ...
    public Condition convertUpdateDto(String code,
                                      String system,
                                      String display,
                                      String patient_id,
                                      LocalDate recorded_date,
                                      String priority_code,
                                      String priority_system,
                                      String priority_display,
                                      String conditionid) {
...
//        }).collect(Collectors.toList());
    ...
    private Condition convertDto(String code,
                                 String system,
                                 String display,
                                 String patient_id,
                                 LocalDate recorded_date,
