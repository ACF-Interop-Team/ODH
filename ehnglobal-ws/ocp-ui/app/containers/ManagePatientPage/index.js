...
export class ManagePatientPage extends React.Component {
  ...
  handleSave(patientFormData, actions) {
    ...
    if (patientFormData.emergencyContacts && !isEmpty(patientFormData.emergencyContacts)) {
      const emergencyContactsMapped = patientFormData.emergencyContacts.map(
        (item) => {
          ...
          const selectedRelationshipTypes = relationshipTypes.find(
            (itemRelationshipType) => itemRelationshipType.code === item.relationshipCode
          );
  ...
  render() {
    ...
    if (patient && patient.id && emergencyContact && !isEmpty(emergencyContact)) {
      const phoneIndex =
        emergencyContact.telecoms &&
        emergencyContact.telecoms.findIndex((item) => item.system === 'phone');

      let emergencyContactMapped = {
        ...emergencyContact,
      };

      if (phoneIndex > -1) {
        const filteredTelecoms = emergencyContact.telecoms.filter(
          (item, index) => index !== phoneIndex
        );

        const phone = emergencyContact.telecoms[phoneIndex];

        emergencyContactMapped = {
          ...emergencyContactMapped,
          telecoms: filteredTelecoms,
          ...phone,
        };
      }

      emergencyContacts.push(emergencyContactMapped);
    }
