...
function TaskTable({
  ...
  function getInitialInterviewBtnRender(task) {
    return (
      <AddNewItemSpan
        onClick={() => {
          console.log('initial interview btn');
          if (onSpecialClick) onSpecialClick(task);
        }}
      >
  ...
  function createTableRows(task, isPatientRole) {
    ...
    const initialInterviewActivityDefinitionRef =
      env_vars.REACT_APP_FHIR_INITIAL_INTERVIEW_ACTIVITY_DEFINITION_REF;

    const displayInitialInterviewBtn =
      task.definition && task.definition.display
        ? task.definition.display === 'initial-interview'
        : false;
    ...
    const calculatedHighlightColor = getHighlightColor(
      executionPeriod && executionPeriod.end
    );

    let backgroundColorTableRowColumn =
      calculatedHighlightColor.backgroundColor;
    let colorTableRowColumn = calculatedHighlightColor.color;
    ...
    return (
      ...
      >
        <StyledTableRowColumn
          onClick={() => {
            if (!displayInitialInterviewBtn && !displayNatWebTaskDetailsBtn)
              onTaskClick(task);
          }}
        >
          {displayInitialInterviewBtn && getInitialInterviewBtnRender(task)}{' '}
          {displayNatWebTaskDetailsBtn && getNatWebTaskDetailsBtnRender(task)}{' '}
