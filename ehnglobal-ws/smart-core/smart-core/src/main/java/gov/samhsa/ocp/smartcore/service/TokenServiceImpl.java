...
import gov.samhsa.ocp.smartcore.service.dto.*;
...
public class TokenServiceImpl implements TokenService {
    ...
    public TokenResponseDto getToken(Optional<String> basicAuth,
        ...
        builder.append("client_id=").append(clientId.get()).append("&");
        ...
        try {
            ...
            try {
                ...
                //context.put("scope", "patient/Patient.read patient/Consent.* launch launch/patient openid profile user/Organization.read user/Practitioner.read user/PractitionerRole.read user/StructureDefinition.read user/Provenance.write patient/CareTeam.read user/ValueSet.read $expand");
...
//                .map(auth -> oAuth2TokenRestClient.getToken("authorization_code", code, redirectUri, clientId.get(), builder.toString()))
//                .orElseGet(() -> clientId
//                        .map(client -> {
...
//                            return oAuth2TokenRestClient.getToken(BASIC_AUTH_PREFIX + encodedBasic, "authorization_code", code, redirectUri, builder.toString());
//                            return oAuth2TokenRestClient.getToken("authorization_code", code, redirectUri, clientId.get(), builder.toString());
//                        })
...
//                final LaunchResponseDto launchResponse = modelMapper.map(launch_iter, LaunchResponseDto.class);
...
//        });
        ...
        if(responseDtoList.size() > 0) {
            Optional<LaunchResponseDto> responseDto = responseDtoList.stream().filter(v -> v.getUser().equals(subid)).findAny();
