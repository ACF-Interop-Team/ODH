...
import gov.samhsa.ocp.ocpuiapi.service.dto.tableau.TicketDto;
...
public class LoginController {
    ...
    public void receiver(HttpServletRequest request, HttpServletResponse response) throws com.nimbusds.oauth2.sdk.ParseException, URISyntaxException, IOException, java.text.ParseException, JOSEException, ParseException, net.minidev.json.parser.ParseException, NoSuchAlgorithmException, InvalidKeySpecException {
        // String url_request = "https://dev-ocp.mynjinck.com/ocp-ui-api/receiver" + "?" + request.getQueryString();
    ...
    public RedirectView testlogin() throws URISyntaxException, IOException, com.nimbusds.jose.shaded.json.parser.ParseException, SerializeException, com.nimbusds.oauth2.sdk.ParseException {
...
//        System.out.println(authReqURI);
    ...
    @GetMapping("/ticket")
    public TicketDto getTicket(@RequestParam(value = "email") String email) {
        StringBuilder parameters = new StringBuilder();
        StringBuilder contentbody = new StringBuilder();

        TicketDto ticketDto = new TicketDto();

        parameters.append("username=").append(email);
        byte[] postdata = parameters.toString().getBytes(StandardCharsets.UTF_8);

        StringBuilder url_remote = new StringBuilder();
        url_remote.append("http://").append(remote_ip).append("/trusted");

        log.info("URL: "+url_remote.toString());
        log.info("USERNAME: "+parameters.toString());

        try {
            URL url = new URL(url_remote.toString());
            ...
            log.info("RESPONSE TABLEAU: "+contentbody.toString());

            ticketDto.setTicket(contentbody.toString());

        } catch (IOException e) {
            log.info("Error: "+e.getMessage());
            throw new BadRequestException("Cannot perform Request: "+e.getMessage());
        } finally {
            connection.disconnect();
        }
